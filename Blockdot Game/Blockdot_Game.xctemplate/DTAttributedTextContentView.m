					...
					if (_delegateFlags.delegateSupportsCustomViewsForAttachments || _delegateFlags.delegateSupportsGenericCustomViews)
					{
						...
						else
						{
							...
							if (attachment)
							{
								...
								{
									newCustomAttachmentView = [self.delegate attributedTextContentView:self viewForAttachment:attachment frame:frameForSubview];
								}
								else
								{
									NSAttributedString *string = [layoutString attributedSubstringFromRange:stringRange]; 
									newCustomAttachmentView = [self.delegate attributedTextContentView:self viewForAttributedString:string frame:frameForSubview];
								}
					...
					if (linkURL && (_delegateFlags.delegateSupportsCustomViewsForLinks || _delegateFlags.delegateSupportsGenericCustomViews))
					{
						...
						else
						{
							...
							if (_delegateFlags.delegateSupportsCustomViewsForLinks)
							{
								...
								newCustomLinkView = [self.delegate attributedTextContentView:self viewForLink:linkURL identifier:guid frame:frameForSubview];
							}
							else if (_delegateFlags.delegateSupportsGenericCustomViews)
							{
								NSAttributedString *string = [layoutString attributedSubstringFromRange:stringRange]; 
								newCustomLinkView = [self.delegate attributedTextContentView:self viewForAttributedString:string frame:frameForSubview];
							}
...
- (void)drawLayer:(CALayer *)layer inContext:(CGContextRef)ctx
{
	...
	SYNCHRONIZE_START(LAYOUTSTRING)
	{
		...
		{
			[self.delegate attributedTextContentView:self didDrawLayoutFrame:self.layoutFrame inContext:ctx];
		}
	}
	SYNCHRONIZE_END(LAYOUTSTRING)
}
...
- (void)setDelegate:(id<DTAttributedTextContentViewDelegate>)delegate
{
	...
	_delegateFlags.delegateSupportsCustomViewsForAttachments = [self.delegate respondsToSelector:@selector(attributedTextContentView:viewForAttachment:frame:)];
	_delegateFlags.delegateSupportsCustomViewsForLinks = [self.delegate respondsToSelector:@selector(attributedTextContentView:viewForLink:identifier:frame:)];
	_delegateFlags.delegateSupportsGenericCustomViews = [self.delegate respondsToSelector:@selector(attributedTextContentView:viewForAttributedString:frame:)];
	_delegateFlags.delegateSupportsNotificationAfterDrawing = [self.delegate respondsToSelector:@selector(attributedTextContentView:didDrawLayoutFrame:inContext:)];
