...
- (id)initWithCTFontDescriptor:(CTFontDescriptorRef)ctFontDescriptor
{
	...
	if (self)
	{
		...
		[self setFontAttributes:(__bridge id)dict];
		...
		self.fontFamily = (__bridge id)familyName;
...
- (id)initWithCTFont:(CTFontRef)ctFont
{
	...
	if (self)
	{
		...
		if (cfStr)
		{
			self.fontFamily = (__bridge NSString *)cfStr;
...
- (CTFontRef)newMatchingFont
{
	...
	{
		// cache it
		[fontCache setObject:(__bridge id)matchingFont forKey:cacheKey];	
	}
...
- (void)normalizeSlow
{
	...
	CTFontDescriptorRef fontDesc = nil; CTFontDescriptorCreateWithAttributes((__bridge CFDictionaryRef)attributes);
	...
	if (fontDesc)
	{
		...
		if (matchingDesc)
		{
			...
			if (family)
			{
				self.fontFamily = (__bridge id)family;
				CFRelease(family);
			}
			
			if (attributes)
			{
				[self setFontAttributes:(__bridge id)attributes];
...
- (CTFontRef)newMatchingFontSlow
{
	...
	CTFontDescriptorRef fontDesc = CTFontDescriptorCreateWithAttributes((__bridge CFDictionaryRef)fontAttributes);
