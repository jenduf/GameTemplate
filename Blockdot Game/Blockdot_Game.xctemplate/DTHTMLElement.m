...
- (NSDictionary *)attributesDictionary
{
	...
	if (textAttachment)
	{
		...
		CTRunDelegateRef embeddedObjectRunDelegate = createEmbeddedObjectRunDelegate((__bridge void *)textAttachment);
		[tmpDict setObject:(__bridge id)embeddedObjectRunDelegate forKey:(id)kCTRunDelegateAttributeName];
	...
	if (shouldAddFont)
	{
		...
		if (!font)
		{
			...
			if (font)
			{
				[self.fontCache setObject:(__bridge id)font forKey:key];
				CFRelease(font);
			}
		}
	...
	if (paragraphStyle)
	{
		...
		[tmpDict setObject:(__bridge id)newParagraphStyle forKey:(id)kCTParagraphStyleAttributeName];
	...
	else
	{
		...
		else
		{
			if ([self.fontDescriptor supportsNativeSmallCaps])
			{
				...
				[smallAttributes setObject:(__bridge id)smallerFont forKey:(id)kCTFontAttributeName];
...
- (NSAttributedString *)prefixForListItem
{
	...
	{
		CTParagraphStyleRef newParagraphStyle = [self.paragraphStyle createCTParagraphStyle];
		[attributes setObject:(__bridge id)newParagraphStyle forKey:(id)kCTParagraphStyleAttributeName];
		CFRelease(newParagraphStyle);
	}
	...
	if (prefix)
	{
		...
		if (image)
		{
			...
			// need run delegate for sizing
			CTRunDelegateRef embeddedObjectRunDelegate = createEmbeddedObjectRunDelegate((__bridge void *)attachment);
			[attributes setObject:(__bridge id)embeddedObjectRunDelegate forKey:(id)kCTRunDelegateAttributeName];
			CFRelease(embeddedObjectRunDelegate);
...
- (void)applyStyleDictionary:(NSDictionary *)styles
{
	...
	if (fontSize)
	{
		...
		{
			fontDescriptor.pointSize = self.parent.fontDescriptor.pointSize;
		}
		else
		{
...
{
	if (!self.parent || [self.parent isMeta] || !self.parent.tagName) // default tag has no tag name
	{
		return NO;
	}
...
- (id)valueForKeyPathWithInheritance:(NSString *)keyPath
{
	...
	{
		return [self.parent valueForKeyPathWithInheritance:keyPath];
	}
	...
	if ([value isKindOfClass:[NSNumber class]])
	{
		...
		if (([number integerValue]==0) && self.parent)
		{
			return [self.parent valueForKeyPathWithInheritance:keyPath];
		}
	}
	...
	if ([value isKindOfClass:[NSString class]])
	{
		...
		if ([string isEqualToString:@"inherit"] && self.parent)
		{
			return [self.parent valueForKeyPathWithInheritance:keyPath];
		}
	}
...
- (DTHTMLElementFontVariant)fontVariant
{
	if (fontVariant == DTHTMLElementFontVariantInherit)
	{
		if (self.parent)
		{
			return self.parent.fontVariant;
		}
...
- (NSString *)path
{
	if (self.parent)
	{
		return [[self.parent path] stringByAppendingFormat:@"/%@", self.tagName];
	}
...
- (NSInteger)listCounter
{
	...
	if (_listCounter == NSIntegerMin)
	{
		...
		if ([tagName isEqualToString:@"li"])
		{
			...
			NSInteger counter = 1;
			NSUInteger numChildren = [self.parent.children count];
			for (NSInteger i = 0; i < numChildren; i++)
			{
				...
				DTHTMLElement *child = [self.parent.children objectAtIndex:i];
