					...
					if (needsListItemStart && [tagContents length] > 0 && ![tagContents isEqualToString:@" "])
					{
						...
						if (prefixString)
						{
							...
							if (nextParagraphAdditionalSpaceBefore>0)
							{
								...
								[finalAttributes setObject:(__bridge id)newParagraphStyle forKey:(id)kCTParagraphStyleAttributeName];
	...
	for (NSString *oneParagraph in paragraphs)
	{
		...
		while (index < NSMaxRange(paragraphRange))
		{
			...
			if (attachment)
			{
				...
				{
					NSString *newKey = [oneKey stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
					NSString *value = [[tmpAttributes objectForKey:newKey] stringByAddingHTMLEntities];
					[retString appendFormat:@" %@=\"%@\"", newKey, value];
				}
